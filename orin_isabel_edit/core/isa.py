...
class IsaNode(Node):
    ...
    def __init__(self):
        ...
        # GRASP VERIFY
        # =====================================
        self.grasp_verify_waiting = False
        self.grasp_verify_wait_start = 0.0
        self.grasp_verify_timeout_sec = 25.0
        self.grasp_verify_cycle_id = None
        self.grasp_verify_last_status = ""
        self.grasp_verify_last_reason = ""

        # =====================================
        # GRASP RETRY
        # =====================================
        # Si la verificacion confirma GRASP_FAILED, ISA devuelve el brazo
        # al pre-grasp del perfil actual y vuelve a intentar APPROACH_BBOX.
        self.grasp_retry_count = 0
        self.grasp_retry_pre_wait_sec = 3.0
        self.grasp_retry_reverse_distance_m = 0.10
        self.grasp_retry_reverse_speed = -0.035
        self.grasp_retry_reverse_max_sec = 4.0
        self.grasp_retry_reverse_start_time = 0.0
        self.grasp_retry_reverse_done = False
        self.grasp_retry_reverse_x0 = 0.0
        self.grasp_retry_reverse_y0 = 0.0
        self.grasp_retry_pre_ready_time = 0.0
        self.grasp_retry_bbox_after_time = 0.0
        self.grasp_retry_last_wait_log = 0.0
        self.last_arm_bbox_time = 0.0
        ...
        self.grasp_verify_cmd_pub = self.create_publisher(
            String,
            '/arm_cam_grasp_verify_command',
            10
        )
    ...
    def _grasp_retry_reverse_traveled(self):
        dx = self.odom_x - self.grasp_retry_reverse_x0
        dy = self.odom_y - self.grasp_retry_reverse_y0
    ...
    def send_grasp_verify_command(self, command: str):
        command = str(command).strip().upper()

        self.grasp_verify_cmd_pub.publish(
            String(data=command)
        )

        self.get_logger().warn(
            f"[ISA] GRASP_VERIFY CMD -> {command}"
        )

    def reset_grasp_verify_wait(self):
        self.grasp_verify_waiting = False
        self.grasp_verify_wait_start = 0.0
        self.grasp_verify_cycle_id = None
        self.grasp_verify_last_status = ""
    ...
    def start_wait_grasp_verify(self):
        self.grasp_verify_waiting = True
        self.grasp_verify_wait_start = time.time()
        self.grasp_verify_last_status = ""
        self.grasp_verify_last_reason = ""

        self.set_state("WAIT_GRASP_VERIFY")

        self.get_logger().warn(
            "[ISA] esperando resultado de /arm_cam_grasp_verify"
        )
    ...
    def bbox_cb(self, msg):
        try:
            ...
            self.last_arm_bbox_time = time.time()
    ...
    def grasp_verify_cb(self, msg):
        try:
            data = json.loads(msg.data)
        except Exception as e:
            self.get_logger().warn(
                f"[ISA] error leyendo /arm_cam_grasp_verify: {e}"
            )
            return

        status = str(data.get("status", "")).strip().upper()
        reason = str(data.get("reason", "")).strip()
        cycle_id = data.get("cycle_id", None)

        if not status:
            return

        if status == "READY" and reason == "NEW_GRASP":
            self.grasp_verify_cycle_id = cycle_id

        self.grasp_verify_last_status = status
        self.grasp_verify_last_reason = reason
        ...
        if status not in terminal_statuses:
            if self.state == "WAIT_GRASP_VERIFY":
                self.get_logger().warn(
                    f"[ISA] GRASP_VERIFY status={status} reason={reason}"
                )
            return

        if self.state != "WAIT_GRASP_VERIFY":
            self.get_logger().warn(
                f"[ISA] ignoro GRASP_VERIFY status={status} "
                f"state={self.state} reason={reason}"
            )
            return

        if (
            self.grasp_verify_cycle_id is not None and
            cycle_id is not None and
            cycle_id != self.grasp_verify_cycle_id
        ):
            self.get_logger().warn(
                f"[ISA] ignoro GRASP_VERIFY ciclo viejo "
                f"status={status} cycle={cycle_id} "
                f"current={self.grasp_verify_cycle_id}"
            )
            return
        ...
        if status == "GRASP_OK":
            ratio = data.get("area_ratio", -1.0)

            self.get_logger().warn(
                f"[ISA] GRASP_VERIFY OK reason={reason} "
                f"ratio={ratio} -> PATROL route_3_point_3"
            )

            self.reset_grasp_verify_wait()
            self.patrol_route_3_point_3()
            self.set_state("GOING_TO_BIN")
            return

        self.get_logger().warn(
            f"[ISA] GRASP_VERIFY FAILED status={status} reason={reason}"
        )

        # GRASP_FAILED significa que el intento fisico fallo: volvemos al
        # pre-grasp y repetimos el BBOX approach con una referencia nueva.
        if status == "GRASP_FAILED":
            self.start_grasp_retry(
                verify_status=status,
                verify_reason=reason
            )
            return

        # Errores del verificador no se reintentan como si fueran un fallo
        # mecanico del grasp. Se dejan visibles para diagnostico.
        self.reset_grasp_verify_wait()
        self.set_state("GRASP_VERIFY_FAILED")
    ...
    def main_loop(self):
        ...
        # GRASP RETRY GATE
        # =================================================
        # Despues de un GRASP_FAILED el brazo vuelve a pre_grasp mientras
        # la base retrocede un poco para recuperar margen visual. No se
        # permite arrancar BBOX hasta completar reversa + espera de pose.
        if self.state == "GRASP_RETRY_BACKING_PRE":
            reverse_traveled = self._grasp_retry_reverse_traveled()
            pre_ready = now >= self.grasp_retry_pre_ready_time
            reverse_timeout = (
                self.grasp_retry_reverse_start_time > 0.0 and
                (now - self.grasp_retry_reverse_start_time) >=
                    self.grasp_retry_reverse_max_sec
            )

            if not self.grasp_retry_reverse_done:
                if (
                    reverse_traveled >= self.grasp_retry_reverse_distance_m or
                    reverse_timeout
                ):
                    self.grasp_retry_reverse_done = True
                    self.stop_base_burst(count=4, interval=0.02)
                    self.get_logger().warn(
                        "[ISA] GRASP RETRY reversa lista "
                        f"{reverse_traveled:.3f}m/"
                        f"{self.grasp_retry_reverse_distance_m:.3f}m "
                        f"timeout={reverse_timeout}"
                    )
                else:
                    twist = Twist()
                    twist.linear.x = self.grasp_retry_reverse_speed
                    self.cmd_pub.publish(twist)

            if self.grasp_retry_reverse_done and pre_ready:
                self.stop_base_burst(count=4, interval=0.02)
                self.grasp_retry_bbox_after_time = now
                self.grasp_retry_last_wait_log = 0.0

                # No permitimos que el siguiente approach use area vieja
                # del grip/lift anterior.
                self.approach.update_area(0.0)
                self.arm_cam_approach.reset_tracking()

                self.get_logger().warn(
                    "[ISA] GRASP RETRY pre-grasp + reversa listos -> "
                    "esperando BBOX NUEVO antes de mover la base"
                )
                self.set_state("GRASP_RETRY_WAIT_BBOX")
                return

            if (now - self.grasp_retry_last_wait_log) >= 0.8:
                self.grasp_retry_last_wait_log = now
                self.get_logger().warn(
                    "[ISA] GRASP RETRY preparando reintento "
                    f"reverse={reverse_traveled:.3f}/"
                    f"{self.grasp_retry_reverse_distance_m:.3f}m "
                    f"pre_ready={pre_ready} "
                    f"timeout={reverse_timeout}"
                )

            return

        if self.state == "GRASP_RETRY_WAIT_BBOX":
            self.stop_base()

            if self.last_arm_bbox_time > self.grasp_retry_bbox_after_time:
                self.get_logger().warn(
                    "[ISA] GRASP RETRY BBOX NUEVO recibido desde pre-grasp "
                    "-> START BBOX APPROACH"
                )
                self.start_bbox_approach()
                return

            if (now - self.grasp_retry_last_wait_log) >= 1.0:
                self.grasp_retry_last_wait_log = now
                self.get_logger().warn(
                    "[ISA] GRASP RETRY esperando /arm_cam_bbox fresco; "
                    "base detenida"
                )
        ...
        if self.state == "WAIT_GRASP_VERIFY":
            if (
                self.grasp_verify_waiting and
                (now - self.grasp_verify_wait_start) >
                    self.grasp_verify_timeout_sec
            ):
                self.stop_base()
                self.get_logger().warn(
                    "[ISA] GRASP_VERIFY timeout; no voy al bin"
                )
                self.reset_grasp_verify_wait()
                self.set_state("GRASP_VERIFY_TIMEOUT")
                return
    ...
    def execute_final_grasp(self):
        ...
        self.stop_base()
        self.set_state("FINAL_GRASPING")

        if not self.grasp.grip_final():
            self.get_logger().warn(
                "[ISA] grip final ya habia sido enviado"
            )

        time.sleep(self.grasp.lift_delay_sec)
        self.send_grasp_verify_command("GRASP_DONE")

        if self.grasp.lift_after_grip():
            time.sleep(self.grasp.lift_delay_sec)
            self.send_grasp_verify_command("LIFT_DONE")
            self.start_wait_grasp_verify()
            return

        self.get_logger().warn(
            "[ISA] no hay lift pose; cancelo verificacion"
        )
        self.set_state("GRASP_VERIFY_FAILED")
    ...
    def start_grasp_retry(self, verify_status="GRASP_FAILED", verify_reason=""):
        """
        Reintenta el grasp sin reiniciar el ciclo completo:
          1) detiene cualquier fuente de movimiento de la base,
          2) limpia la verificacion anterior,
          3) resetea flags de GraspModule,
          4) vuelve al pre-grasp del objeto actual,
          5) retrocede 10 cm mientras el brazo llega/estabiliza,
          6) espera un /arm_cam_bbox NUEVO desde esa pose,
          7) solo entonces inicia otra vez APPROACH_BBOX.

        El nuevo APPROACH_BBOX manda NEW_GRASP, por lo que la siguiente
        verificacion usa una referencia/cycle_id nuevo.
        """

        if self.state != "WAIT_GRASP_VERIFY":
            self.get_logger().warn(
                f"[ISA] GRASP RETRY ignorado desde state={self.state}"
            )
            return

        # Cortar explicitamente cualquier modulo que pudiera publicar cmd_vel.
        self.approach.stop()
        self.align.stop()
        self.arm_cam_approach.stop()
        ...
        self.reset_grasp_verify_wait()
        self.send_grasp_verify_command("RESET")

        self.grasp_retry_count += 1

        self.get_logger().warn(
            f"[ISA] GRASP RETRY #{self.grasp_retry_count} "
            f"status={verify_status} reason={verify_reason} "
            f"class={self.target_class}"
        )

        # El primer intento ya marco pre/grip/lift como enviados.
        # Hay que limpiar esos flags para que el siguiente intento vuelva
        # a publicar grip y lift realmente.
        self.grasp.reset()
        self.grasp.set_profile(self.target_class)
        ...
        # Invalida cualquier bbox anterior al retorno a pre-grasp.
        self.last_arm_bbox_time = 0.0
        self.approach.update_area(0.0)
        self.arm_cam_approach.reset_tracking()

        self.grasp_retry_reverse_x0 = self.odom_x
        self.grasp_retry_reverse_y0 = self.odom_y
        self.grasp_retry_reverse_start_time = time.time()
        self.grasp_retry_reverse_done = False

        self.set_state("GRASP_RETRY_BACKING_PRE")

        # El brazo vuelve a pre-grasp mientras la base abre 10 cm de margen.
        self.grasp.pre_grasp()
        self.profile_prepared = True

        self.grasp_retry_pre_ready_time = (
            self.grasp_retry_reverse_start_time +
            self.grasp_retry_pre_wait_sec
        )
        self.grasp_retry_bbox_after_time = 0.0
        self.grasp_retry_last_wait_log = 0.0

        self.get_logger().warn(
            f"[ISA] GRASP RETRY -> {self.grasp.pre_grasp_pose}; "
            f"reverse={self.grasp_retry_reverse_distance_m:.2f}m "
            f"speed={self.grasp_retry_reverse_speed:.3f}; "
            f"esperando {self.grasp_retry_pre_wait_sec:.1f}s "
            "para estabilizar brazo"
        )
    ...
    def start_bbox_approach(self):
        ...
        self.arm_not_ok_counter = 0
        self.reset_grasp_verify_wait()
        self.send_grasp_verify_command("NEW_GRASP")
